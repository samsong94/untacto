...
router.get('/', function (req, res, next) {
  ...
  var sql_count =
    "select count(*) as cnt from survey where companyId='" + companyId + "'";
  ...
  //count kiosk list
  var k_cnt;
  connection.query(sql_kiosk_count, function (
    err_k_cnt,
    rows_k_cnt,
    fields_k_cnt,
  ) {
  ...
  connection.query(sql_company, function (
    err_company,
    rows_company,
    fields_company,
  ) {
  ...
  connection.query(sql_list, function (err_list, rows_list, fields_list) {
    if (!err_list) {
      ...
      if (survey_list.length) res.json(survey_list);
      else res.json(null);
    } else {
      throw err_list;
    }
